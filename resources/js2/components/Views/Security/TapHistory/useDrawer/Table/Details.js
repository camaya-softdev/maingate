...
const Details = ({ data: scanData }) => {
  const rowSelection = {
    selectedRowKeys: isEmpty(scanData) ? [] :
      keys(pickBy(scanData.checklists.booking, value => value === true)).map(key => Number(key)),
  };
      ...
      render: function render(id) {
        return <div className="text-center">
          <Form.Item name={`action_taken.booking.${id}`} className="m-0"
            initialValue={`${scanData.action_taken?.booking[id] || ''}`}>
  ...
  const data = [
    ...
    },
    {
      ...
      item: !isEmpty(scanData) &&
        values(scanData.booking_guest_vehicles).map((vehicle, index) =>
          <div key={vehicle.id}>
