...
const Guests = ({ data: scanData }) => {
  const rowSelection = {
    selectedRowKeys: isEmpty(scanData) ? [] :
      keys(
        pickBy(
          scanData.security_check.checklists.guests, value => value === true
        )
      ).map(key => Number(key)),
  };
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      render: function render(id) {
        return startCase(scanData.guests_status[id]);
      }
    },
    {
      title: 'Vehicle',
      render: function render(text, record) {
        ...
        const guestAdditionalVehicles = scanData.security_check.additional_vehicles;
        const guestVehicle = scanData.security_check.guest_vehicles[record.id];

        if (guestVehicle && /^additional-/.test(guestVehicle)) {
          const vehicleIndex = guestVehicle.replace('additional-', '');
      ...
      render: function render(id) {
        return <div className="text-center">
          <Form.Item name={`action_taken.guests.${id}`} className="m-0"
            initialValue={`${scanData.security_check.action_taken?.guests[id] || ''}`}>
