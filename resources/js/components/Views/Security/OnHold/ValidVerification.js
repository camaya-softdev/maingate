...
const Verification = () => {
  ...
  const scanData = useAtomValue(onHoldDataAtom);
  const setBarrier = useUpdateAtom(barrierAtom);
  const scanHoldingAreaAPI = scanHoldingArea();
  const scanValidAPI = scanValid('/api/security-checks');
  ...
  const onHoldDataReset = useResetAtom(onHoldDataAtom);

  const processGuestAdditionalVehicles = async guestAdditionalVehicles => {
    for (const vehicle of guestAdditionalVehicles) {
      const updatedVehicle = {
        booking_reference_number: scanData.details.booking_reference_number,
        ...vehicle,
      };
  ...
  const onSubmit = (status = 'validated') => {
    let arg = {
      ...
      booking_reference_number: scanData.details.booking_reference_number,
      reference_number: scanData.details.reference_number,
      ...
      guestVehicles.map(vehicle => {
        if (/^additional/.test(vehicle.id)) {
          putGuestVehicleAPI.mutate({
            values: vehicle,
            bookingReferenceNumber: scanData.details.booking_reference_number
          });
        } else {
