...
const OpenBarrier = () => {
  ...
  useEffect(() => {
    ...
    if (isEmpty(scanDataMobile) && !isEmpty(scanDataHoa)) {
      setIsScanDataEmpty(true);
    } else if (isEmpty(scanDataHoa) && !isEmpty(scanDataMobile)) {
      setIsScanDataEmpty(true);
    }

    if (isScanDataEmpty && !scanDataMobile) {
      setBarrier(false);
  ...
  }, [redirectKiosk, scanDataMobile, scanDataHoa, isScanDataEmpty]);
